...
/**
  ...
  */
case class Header(path: String, label: Node, group: Option[String], includeIndexes: List[Int])
    extends Linkable
...
object Page {
  ...
  def convertPage(convertPath: String => String)(page: Index.Page): Page = {
    ...
    val (h1, subheaders) = page.headers match {
      case h :: hs =>
        (
          Header(h.label.path, h.label.markdown, h.label.group, h.label.includeIndexes),
          h.children ++ hs
        )
      case Nil => (Header(targetPath, new SpecialTextNode(targetPath), None, Nil), Nil)
    }
    val headers =
      subheaders map (_ map (h => Header(h.path, h.markdown, h.group, h.includeIndexes)))
    Page(
      page.file,
      targetPath,
      rootSrcPage,
  ...
  def allPaths(pages: Forest[Page]): List[String] = {
    @tailrec
    def collect(location: Option[Location[Page]], paths: List[String] = Nil): List[String] =
      location match {
  ...
  case class Properties(props: Map[String, String]) {
    ...
    def convertToTarget(convertPath: String => String): String => String =
      (path: String) =>
        replaceFile(props.get(Properties.DefaultOutMdIndicator))(path) getOrElse convertPath(path)

    // TODO: give the target suffix ".html" in a more general way
    private def replaceFile(prop: Option[String], targetSuffix: String = ".html")(
        path: String
    ): Option[String] = prop match {
...
object Path {
  ...
  /**
    ...
    */
  def toUnixStyleRootPath(pathString: String): String =
    toUnixStyleRootPath(NioPaths.get(pathString))
  ...
  def toUnixStyleRootPath(path: NioPath): String = {
    ...
    val fullPathString =
      if (fullPathWithDriveLetter.getRoot ne null)
        File.separator + fullPathWithDriveLetter.getRoot
          .relativize(fullPathWithDriveLetter)
          .toString
      else
  ...
  def generateTargetFile(
      localPath: String,
      globalPageMappings: Map[String, String]
  ): String => String = {
